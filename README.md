...
**Recording new fixtures from a device:** record a clean session and a seeded-errors session in Recite on the phone (logs land in the app's `Documents/recite-logs/`), then run `npm --prefix app run recite:pull` (`-- --dry-run` to preview). It auto-discovers the paired iPhone (or takes `--device=<udid>`), copies the newest logs into [`samples/recite-logs/`](samples/recite-logs/), prints the `parse-recite-log.mjs` analysis, and scaffolds a `<timestamp>_labels.json` per log with the reconciled reds pre-filled as `notIntentional[]` ("TODO confirm" notes). Confirm the labels — move seeded errors into `intentional[]` — then add a `recite:golden-device`-style gate script (`recite-align-test.mjs <log> <labels> --max-false-subs=0`) to `app/package.json`.
